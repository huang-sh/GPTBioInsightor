...
def test_query_celltype(request):
...
def test_get_celltype(request):
    model = request.config.getoption("--model", default="qwen2-1.5b-instruct")
    gene_dic = {
        "gs1": ["CD19", "MS4A1", "CD79A", "CD79", "CCR7"],
        "gs2": ["CD3", "CD3E", "CD8", "CD3D", "25CD" ]}
    background = "Human blood"
    celltype_dic = get_celltype(gene_dic, background=background, out="test.md" ,provider=provider, model=model,base_url=base_url)
